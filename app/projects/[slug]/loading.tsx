...
export default function ProjectLoading() {
	return (
		<section className="py-12 md:py-20 border-t border-border-light bg-transparent">
			<Container>
				{/* Back Link Skeleton */}
				<div className="h-4 w-32 bg-skeleton rounded mb-8 animate-pulse" />
				...
				{/* Image Skeleton */}
				<div className="mb-12 h-96 bg-skeleton border border-border-light rounded animate-pulse" />
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
					...
					<div className="lg:col-span-2 space-y-12">
						{/* Tech Stack Section */}
						<div>
							<div className="h-6 w-32 bg-skeleton rounded mb-4 animate-pulse" />
							<div className="flex flex-wrap gap-2">
								{[...Array(5)].map((_, i) => (
									<div
										// biome-ignore lint/suspicious/noArrayIndexKey: static skeleton array, no reordering
										key={i}
										className="h-10 w-24 bg-skeleton rounded animate-pulse"
									/>
								))}
							</div>
						</div>
						...
						{/* Tags Section */}
						<div>
							<div className="h-6 w-16 bg-skeleton rounded mb-4 animate-pulse" />
							<div className="flex flex-wrap gap-2">
								{[...Array(3)].map((_, i) => (
									<div
										// biome-ignore lint/suspicious/noArrayIndexKey: static skeleton array, no reordering
										key={i}
										className="h-8 w-16 bg-skeleton rounded animate-pulse"
									/>
								))}
							</div>
						</div>
					...
					<div className="lg:col-span-1">
						<div className="bg-white border border-border-light p-6 shadow-sm space-y-4">
							<div className="h-6 w-24 bg-skeleton rounded animate-pulse" />
							{[...Array(2)].map((_, i) => (
								<div
									// biome-ignore lint/suspicious/noArrayIndexKey: static skeleton array, no reordering
									key={i}
									className="h-4 w-32 bg-skeleton rounded animate-pulse"
								/>
							))}
						</div>
					</div>
				...
				<div className="pt-12 border-t border-border-light">
					<div className="h-8 w-48 bg-skeleton rounded mb-6 animate-pulse" />
